;$(function($){
	...
	var ajaxUrl = SystemProp.appServerUrl+"/look/look-stat-category-trend!searchCategoryTrendJson.action";
	...
	var strsd = strDate(startDate),
		...
		firstCondition = {"startDate":strsd,"endDate":stred,"categoryId":$("#categoryId").val(),"itemId":$("#itemId").val()};
	$eDate.val(stred);
	$sDate.val(strsd);
	...
	$("#categoryId").unbind('change').change(function(e){
		var categoryId = $("#categoryId").val();
		getItem(categoryId);
	});
	getItem($("#categoryId").val());
	function getItem(categoryId)
	{
		$("#itemId").html("<option value='0'>全部</option>");
		$.ajax({
			url : SystemProp.appServerUrl + "/look/look-item!getItemByCategoryIdJson.action",
			type : "POST",
			async : false,
			dataType : "json",
			...
			success : function(rs){
				...
				{
					if(rs.data&&rs.data.itemList)
					{
						$.each(rs.data.itemList,function(e,r){
							$("#itemId").append("<option value='"+r.itemId+"'>"+r.title+"</option>");
						});
					}
