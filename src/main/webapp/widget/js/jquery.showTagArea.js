...
//tagUrl:tag图url
	jQuery.showTagArea = function(setting) {
		...
		var maxHeight;
			if(Math.round(imgHeight*maxWidth/imgWidth)<590){
				maxHeight = Math.round(imgHeight*maxWidth/imgWidth);
			}else{
				maxHeight = 590;
			}
		...
		$tag.attr("src",tagUrl);
		//$(id).height(maxHeight);
		$mask.css({width:maxWidth,height:maxHeight,maxHeight:590,opacity:0.5});
		$bigImg.css({maxHeight:590});
		$mask.css({maxHeight:590});
		...
		if(Math.random()>0.5){
			...
			$showArea.css({width:autoWidth,height:"auto",top:0,left:0,maxHeight:590});
			$showArea.find("img").css({width:autoWidth,height:"auto",maxHeight:590});
		}else{
			$showArea.css({width:showAreaWidth,height:showAreaHeight,maxHeight:590,top:showAreaTop,left:showAreaLeft});
			$tag.css({width:showAreaWidth,height:showAreaHeight,maxHeight:590});
		}
		...
		function viewClick(code){
	    	$.ajax({
					url:SystemProp.domain+"/user-tag!clickJson.action",
					type : "POST",
					data : {"code":code},
					dataType : "json"
			});
