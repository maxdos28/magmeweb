...
<head>
<title><#if creativeList?? && (creativeList?size)&gt;0  >${creativeList.get(0).title!}</#if> - 麦米网Magme</title>
...
<div class="body pageM1 noBg clearFix">
		                    ...
		                    <a href="${systemProp.appServerUrl}/index-detail.action?itemId=${c.id}&type=event" >
		                        ...
		                        <div class="uInfo png">
		                            <div class="userHead png">
		                                <div class="head" href="javascript:void(0)"><img src="${systemProp.publishProfileServerUrl}${c.avatar!}" onerror="this.src='${systemProp.staticServerUrl}/images/head60.gif'" />
		                        ...
		                        <div class="conReply conReplyBig">
		                        	<#list c.fpageEventComment?reverse as cfs >
		                        		...
		                        		<#else>
	                        				...
	                        				<a title="${cfs.user.nickName!}" href="<#if ((session_user)??) && (session_user.id==cfs.user.id) >${systemProp.appServerUrl}/sns/${(session_user.id)!'0'}/<#else>${systemProp.appServerUrl}/sns/u${cfs.user.id}/</#if>" class="head"><img src="${systemProp.profileServerUrl}<#if cfs.user.avatar?? && cfs.user.avatar!="" >${avatarResize(cfs.user.avatar,'46')}</#if>" onerror="this.src='${systemProp.staticServerUrl}/images/head60.gif'"  /></a>
	                        				<p><span>
	        			...
	        			<#list c.creativeEx as ex>
	        				<#if ex.conType==2>
			        			<div class="content conPhoto clearFix">
					            	...
					            	<#if c.activity==true><a href="${systemProp.appServerUrl}/v3/event/20120824/event.html" class="eventName">[十万年薪]</a></#if>
	...
	<div class="sideRight">
    	...
    	<#--<div class="con conMorePic">  SEO删除　下次还原　
            		...
            		<a <#if ((session_user)??) && (session_user.id==ncv.userId) >url="${systemProp.appServerUrl}/sns/user-index.action"<#else>url="${systemProp.appServerUrl}/sns/u${ncv.userId}/" </#if> href='javascript:void()' />
            			<span>
            				<#list ncv.ex as ex >
            					<#if ex_index==0>
            						<#if ex.conType==2>
            							<img src="${systemProp.staticServerUrl}${avatarResize(ex.imgPath!,'500')}"/>
            						<#else>
            							<img src="${ex.imgPath!}" />
            						</#if>
            					</#if>
