<body>
	<div class="conB">
		...
		<fieldset>
        	<div>
            	...
            	<em>
            		<#if session_admin??>
            			<input type="text" class="input g100" id="appName"/>
            		<#else>
	                 <select id="appId">
	                      <#if apps?? && (apps?size>0)>
		            		  <#list apps as app>
		            		   	<option value="${app.id}" <#if appId?? && appId=app.id> selected </#if>  >${app.name}</option>
		            		  </#list>
		            		</#if>
	                 </select>
		           </#if>
  ...
  <div class="conB">
   ...
   <script type="text/javascript" src="${systemProp.staticServerUrl}/v3/js/stat/dmreachable.js"></script>
</body>