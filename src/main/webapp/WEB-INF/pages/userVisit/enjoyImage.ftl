    	...
    	<div class="tagWallInner" id="tagWall">
        	<div class="item itemUser itemShowBtn">
                ...
                <ul class="clear listB clearFix">
                    <li><a class="current" name="fav" href="${systemProp.appServerUrl}/user-visit!index.action?userId=${userId!""}">TA喜欢的切米</a></li>
			  ...
			  <li><a name="pic" href="${systemProp.appServerUrl}/user-visit!userImage.action?userId=${userId!""}">TA的图片</a></li>                    
                ...
                <div class="clear scroll-pane">
                    <ul class="listB listTag">
	                	...
	                	<#list tagList as tag>
	                		<#if tag_index+1<=6>
	                		<li><a href="${systemProp.appServerUrl}/user-visit!index.action?userId=${userId!""}&tagName=${encode(tag.name)}" class="tag${tag_index+1} <#if tagName??&&(tagName==tag.name)>current</#if>">${tag.name}</a></li>
	                		<#else>
	                		<li><a href="${systemProp.appServerUrl}/user-visit!index.action?userId=${userId!""}&tagName=${encode(tag.name)}" class="<#if tagName??&&(tagName==tag.name)>current</#if>">${tag.name}</a></li>
	                		</#if>
	                	</#list>                        
                    </ul>
                </div>
