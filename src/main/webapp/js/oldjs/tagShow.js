$(document).ready(function(){
    ...
    var desc = "";
    $("a[tagShare]").unbind('click').live('click',function(e){
    	e.preventDefault();
    	var shareBtn = $(this);
    ...
    $("#goBackPage").click(function(e){
    	...
    	var url = SystemProp.appServerUrl+"user-tag!tagWall.action?begin=0&size="+size+"&scrollX="+scrollX;
    	if(categoryId){
	...
	$(window).scroll(function(){
		...
		if(!scrollTimer && !hasOver){
			scrollTimer = setTimeout(function(){
				var commentNum = $(".clearFix",$("#tagCommentList")).length;
				...
				if(scrollTop > (scrollHeight-100)){
					if(hasComment){
						addComment(commentNum,10,tagId);
					}
				}
	...
	function addComment(begin,size,tagId){
		...
		$.ajax({
			url : SystemProp.appServerUrl+"/user-tag!getTagCommentListJson.action",
			type : "POST",
			async : false,
			data : data,
			success : callback
    ...
    var formSubmit = function(){
    	var url = SystemProp.appServerUrl+"/tag-comment!loginCommentJson.action";
	...
	$("div[title='点击阅读杂志']").live('click',function(){
		var url = $("img[url]").eq(0).attr("url");
		window.open( url,"_magme");
	});
	// click_img-------------------------------------------
	...
	function loadTags(direction){
		...
		var callback = function(result) {
			if (!result)
				return;
			var tagListData = result.data.tagList;
			...
			if (tagListData && length > 0) {
				...
				for ( var i = 0; i < length; i++) {
					...
					tagList.append('<a href="'
							...
							+ '/user-tag!show.action?id=' + tagData.id
							+ '"><img title="' + tagData.title
	...
	//focus-comment------------------------------
	var focus = getUrlValue("focus");
	if(focus && focus == 'comment' ){
		$(window).scrollTop(665);
		$("textarea").focus();
	}
});
