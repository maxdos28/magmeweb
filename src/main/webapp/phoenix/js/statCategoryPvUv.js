;$(function($){
	...
	var ajaxUrl = SystemProp.appServerUrl+"/phoenix/phoenix-stat-category-pv-uv!searchCategoryPvUvJson.action";
	...
	function drawChart(data){
		...
		for(var i=0;i<data.length;i++){
			var adData = data[i];
			mydataArray.push([adData.name,adData.uvNum,adData.pvNum]);
		}
		...
		myChart.setTitle('栏目浏览量报表');
		myChart.setTitleColor('#8E8E8E');
		if(!$("#categoryId").val()||$("#categoryId").val()=="0")
			myChart.setAxisNameX('分类');
		else
		...
		myChart.setLegendForBar(1, 'uv');
		myChart.setLegendForBar(2, 'pv');
	...
	var strsd = strDate(startDate),
		...
		firstCondition = {"startDate":strsd,"endDate":stred,"categoryId":categoryId};
	$eDate.val(stred);
	$sDate.val(strsd);
