$(function() {
	...
	function writePub(rl) {
		...
		for(i=0;i<rl.length;i++)
		{
			...
			trContent += "<td><label><input name='parentCheck' itemId='"+rl[i].id+"' checked type='checkbox' />选择</label></td>";
	...
	$("#searchAdBtn").unbind("click").live("click", function() {
		...
		$.ajax({
			url : SystemProp.appServerUrl + "/ma/ad-trade-new-of-article!searchAdByArticle.action",
			type : "POST",
			async : false,
			dataType : "json",
			data : data,
			success : function(rs){
				...
				{
					writePub(rs.data.adLocationListByArticle);
				}
			}
		});
	});
	...
	$("#saveAdTradeBtn").unbind("click").live("click", function() {
		...
		$.ajax({
			url : SystemProp.appServerUrl + "/ma/ad-trade-new-of-article!saveAdTrade.action",
			type : "POST",
			async : false,
			dataType : "json",
			data : data,
