$(function(){
	...
	var ajaxUrl = SystemProp.appServerUrl + 
			(($("#isVisitor").val() == "1") ? "/user-visit!enjoyImageJson.action" 
					: "/user-center!enjoyImageJson.action");
		...
		$.ajax({
			...
			success : function (rs){
				...
				if(rs.code == 200 && !!rs.data && !!rs.data.userImageList && rs.data.userImageList.length>0){
					...
					var tagStr = '<div class="item">{{if type==1}}<a class="photo" style="${height*200/width+22}px;" href="${SystemProp.appServerUrl}/user-image!show.action?'+(!!tagName ? ('tageName='+tagName+'&') : '')+'imageId=${id}">'+
		            	...
		            	'</div>{{else}}<a clickEventId="" class="photo" style="${height*200/width+22}px;" href="${SystemProp.appServerUrl}/user-image!show.action?id=${id}">'+
		            	'<img height="${height*200/width}px" src="${SystemProp.fpageServerUrl}/event${path200}" /><h5>${title}</h5></a>'+
		            	'<div class="tools"><em favTypeId="eve_${id}" class="iconHeart favCurrent">${enjoyNum}</em>'+
		            	'<em class="iconShare" shareTypeId="eve_${id}">分享</em></div><div class="info">'+
		            	'<p>出自<a href="${SystemProp.appServerUrl}/publish/mag-read.action?id=${issueId}">[&nbsp;${publicationName||""}&nbsp;]</a></p></div>{{/if}}</div>';
