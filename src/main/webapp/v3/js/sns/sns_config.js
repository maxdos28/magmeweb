;$(document).ready(function() {
	...
	$dateInput.DatePicker({
		...
		onChange: function(formated, dates){
			...
			var m=new Date(formated).getMonth()+1;
			var d=new Date(formated).getDate();
			var i=0,j=0;
	...
	$("#doApply").click(function(){
		var data=form2object('applyForm');
		data.imgFileName = data.imgFile;
		if(!data.nameZh || !data.imgFileName || !data.phone || !data.publisher || !data.office){
			alert("请输入完整信息!");
			return;
		}
		if(!isNum(data.phone)){
		...
		var imf=$("#imgFile").val().toLocaleLowerCase();
		 if(imf.match(/^.*(jpg|jpeg|gif|png)$/)==null){
			 alert("请上传工作相关证明图片")
			 return;
		 }
...
//			} else {
				$.ajaxFileUpload( {
					url : SystemProp.appServerUrl+"/sns/info-user!apply.action",
					secureuri : false,
					data : data,
...
//		});
	...
	function validate(callback){
		...
		delete  data.imgFile;
		$.ajax({
			url : SystemProp.appServerUrl + "/sns/info-user!validateDetail.action",
			type : "POST",
			data : data,
			success: callback
		});
	...
	$("#applyMCertification").unbind("click").live("click",function(){
		$("#applyForm input[name=phone]").val($("#editUserInfoForm input[name=phone]").val());
		$("#applyForm input[name=nameZh]").val($("#editUserInfoForm input[name=reserve3]").val());
		$("#applyCertification").fancybox();
		fnHideSelect();
	...
	function modify_userInfo_submit(){
		...
		//允许为空的字段列表
		var allowSpaceArr = ["phone", "occupation", "hobbies", "reserve2", "address"];
		for(var i = 0; i < allowSpaceArr.length; i++ ){
			if(!user[allowSpaceArr[i]]) user[allowSpaceArr[i]] = "";
		}
		...
		var callback = function(result){
			...
			if(code==200){
				...
				}else if(data && data.oldPassword){
					$("#editUserInfoForm").find("#oldPassword").addClass("inputError");
					tipError.html(data.oldPassword).show();
				}else if(data && data.reserve2){
					$("#editUserInfoForm").find("#reserve2").addClass("inputError");
					tipError.html(data.reserve2).show();
				} else {
					alert(result.message);
				}
			}
...
$(function(){
	...
	var t = [];
	function pname(val){
		var url = SystemProp.appServerUrl + "/sns/info-user!pname.action";
		$.ajax({
			url:url,
			type : "POST",
			data : {"publisher":val},
	...
	$("#selectPublisher").smartSearch(jsonFn);
	function jsonFn(val) {
		return pname(val);
	}
