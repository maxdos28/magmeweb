;$(function($){
	...
	var callBack = function(rs){
		var code = rs.code;
		if(code != 200) return;
		if(!rs.data || !rs.data.dmReaderActionList || rs.data.dmReaderActionList.length == 0){
			alert('没有数据');
			return;
		}
		drawChart(rs.data.dmReaderActionList);
	};
	var ajaxUrl = SystemProp.appServerUrl+"/stat/dm-reader-act!queryReaderActJson.action";
	...
	function drawChart(data){
		...
		for(var i=0;i<data.length;i++){
			var adData = data[i];
			mydataArray.push([dateFormat(adData.dataDate),adData.clickPv]);
			mydataArray2.push([dateFormat(adData.dataDate),adData.clickUv]);
		}
		...
		myChart.setLegendForLine('blue', '次数(按pv)');
		myChart.setLegendForLine('green', '次数(按uv)');
		myChart.draw();
	}
	//default 当前时间向前推7天
