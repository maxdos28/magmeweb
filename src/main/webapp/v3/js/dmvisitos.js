;$(function($){
	...
	var callBack = function(rs){
		var code = rs.code;
		if(code != 200) return;
		if(!rs.data || !rs.data.visitOsList || rs.data.visitOsList.length == 0 || rs.data.totalNum*1 == 0){
			alert('没有数据');
			return;
		}
		drawChart(rs.data.visitOsList,rs.data.totalNum);
	};
	var ajaxUrl = SystemProp.appServerUrl+"/stat/dm-visit-os!queryVisitOsJson.action";
	...
	function drawChart(data,num){
		...
		for(var i=0;i<data.length;i++){
			...
			if(i<5){
				ot += scale;
				chartArray.push([listData.os,scale]);
			}else if(i==5){
				chartArray.push(["其他",(100 - ot)]);
			}
			var $tr = $("<tr></tr>").appendTo($tbody);
			$("<td>"+(listData.os||"")+"</td>").appendTo($tr);
