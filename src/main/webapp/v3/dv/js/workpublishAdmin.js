...
//	已发布页面 end
	...
	function pageComm(currentPage){
			...
			if(result.code!=200){
				...
				$.each(result.data.creativeList,function(i,creative){
					...
					if(data.ishome==1){
							...
							trContent += "<td>"+creative.weight+"</td>";
		...
		$.ajax({
			url: SystemProp.appServerUrl+"/new-publisher/work-publish!listM1Json.action",
			type: "POST",
			dataType: "json",
			data: data,
			success: callback
	...
	function pageComm2(currentPage){
		...
		$.ajax({
			url: SystemProp.appServerUrl+"/new-publisher/work-publish!listM1CountJson.action",
			type: "POST",
			dataType: "json",
			async:false,
			data: data,
