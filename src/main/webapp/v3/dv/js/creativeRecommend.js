$(function(){
	...
	$("a[name='activityalbumedit']").live("click",function(){
		...
		var callback = function(rs){
			if(rs.code!=200){
				...
				$("#activityAlbumForm input[name='weights']").val(po.sortVal);
		...
		$.ajax({
			url: SystemProp.appServerUrl+"/new-publisher/creative-recommed!getPojoJson.action",
			type: "POST",
			dataType: "json",
			data: data,
			success: callback
	...
	$("a[name='activityalbumstatus']").live("click",function(){
		...
		$.ajax({
			url: SystemProp.appServerUrl+"/new-publisher/creative-recommed!updateStatusJson.action",
			type: "POST",
			dataType: "json",
			data: data,
			success: callback
	...
	$("#saveActivityAlbumBtn").live("click",function(){
		var url=SystemProp.appServerUrl+"/new-publisher/creative-recommed!updateJson.action";
	...
	function pageComm(currentPage,isFlag){
		...
		var callback =function(result){
			...
			if(result.code!=200){
				...
				$.each(result.data.listaa,function(k,a){
					...
					trContent+="<td><img width='100px' height='40px' src='http://static.magme.com/activityalbum"+a.imagePath+"' /></td>";
					trContent+="<td>"+a.url+"</td>";
					var tpyeValue = "";
					var atype =a.type;
					if(atype==0){tpyeValue="期刊";}
					else if(atype==1){tpyeValue="广告";}
					trContent+="<td>"+tpyeValue+"</td>";
					trContent+="<td>"+a.sortVal+"</td>";
					if(a.createDate){
		...
		$.ajax({
			url: SystemProp.appServerUrl+"/new-publisher/creative-recommed!listJson.action",
			type: "POST",
			dataType: "json",
			data: data,
			success: callback
