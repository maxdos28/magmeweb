...
function readerGoBack(){
	var backUrl = getUrlValue("backUrl").replace(/(&?)curPage=[^&]+/g,"");
	var backPageNo = getUrlValue("backPageNo");
	window.location.href = backUrl+((backUrl.indexOf('?')>-1)?'&':'?')+"curPage="+backPageNo;
}
