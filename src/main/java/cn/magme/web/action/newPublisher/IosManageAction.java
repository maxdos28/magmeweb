 ...
 * @date 2012-09-14
 */
@Results({@Result(name="success",location="/WEB-INF/pages/newPublisher/iosManage.ftl")})
public class IosManageAction extends BaseAction {
	...
	public String listJson(){
		...
		iosApp.setUserId(publisherId);
		iosApp.setUserType(1);
		appList = iosAppService.queryIosAppList(iosApp, 0, 0);
		
		IosPush iosPush = new IosPush();
		iosPush.setUserId(publisherId);
	...
	public String addIosPush(){
		...
		iosPush.setUserId(this.getSessionPublisherId());
		iosPush.setUserType(1);//出版商用户
		if(iosId!=null){
			iosPush.setId(iosId);
			iosPushService.updateIosPush(iosPush);
			this.jsonResult.setCode(JsonResult.CODE.SUCCESS);
			...
			if(status==1){//立即发送消息
				...
				if(appIdArray!=null&&appIdArray.length>0){
					int k =0;
					k=iosPushService.iosPushMessage(1,this.getSessionPublisherId(), content,null,2,appIdArray);
					if(k==0){
