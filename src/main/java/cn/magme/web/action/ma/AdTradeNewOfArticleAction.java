 ...
 * 
 */
@Results({ @Result(name = "success", location = "/WEB-INF/pages/ma/adTradeNewOfArticle.ftl") })
public class AdTradeNewOfArticleAction extends BaseAction {
	...
	private List<Map> adLocationListByArticle;
	 ...
	 * @return
	 */
	public String searchAdByArticle() {
		this.jsonResult = JsonResult.getFailure();
		List<String> padl = padList();
		List<String> phonel = phoneList();
		List<String> kl = keywordList();
		this.adLocationListByArticle = this.maAdLocationService
				.searchAdLocationByItem(devicePad, devicePhone, padl, phonel,
						kl, startDate, endDate, contentStartDate,
						contentEndDate);
		this.jsonResult.put("adLocationListByArticle", adLocationListByArticle);
	...
	public String saveAdTrade() {
		...
		if (StringUtil.isBlank(contentStartDate)
				|| StringUtil.isBlank(contentEndDate)) {
		...
		String[] scriptValueList = (String[]) parameter
				.get("scriptValueList[]");
		if (scriptKeyList == null || scriptKeyList.length == 0
				|| scriptValueList == null || scriptValueList.length == 0
				|| scriptKeyList.length != scriptValueList.length) {
			this.jsonResult.setMessage("广告尺寸或脚本未填写");
			return JSON;
		}
		if (StringUtil.isBlank(adItems)) {
		...
		this.jsonResult = this.maAdTradeService.createAdTradeOfItem(mh, keywords,
				scriptKeyList, scriptValueList, adItems);
		return JSON;
	}
	...
	public List<Map> getAdLocationListByArticle() {
		return adLocationListByArticle;
	}

	public void setAdLocationListByArticle(List<Map> adLocationListByArticle) {
		this.adLocationListByArticle = adLocationListByArticle;
	}
	...
	public String getAdItems() {
		return adItems;
	}

	public void setAdItems(String adItems) {
