 ...
 */
@Results({
	@Result(name="success",location="/WEB-INF/pages/golf/workrelease.ftl"),
	@Result(name="audited",location="/WEB-INF/pages/golf/workaudited.ftl"),
	@Result(name="published",location="/WEB-INF/pages/golf/workpublished.ftl")
})
public class WorkPublishAction extends BaseAction{
	...
	public String execute(){
		...
		if(creativeCategoryList!=null){
			for (CreativeCategory cc : creativeCategoryList) {
				int tempFirst = creativeCategoryRelService.creativeNumByFirstType(cc.getId(),ishome,3);
				cc.setCreativeCount(tempFirst);
				if(cc.getChildCreativeList()!=null){
					for (CreativeCategory ccs : cc.getChildCreativeList()) {
						int tempSecond = creativeCategoryRelService.creativeNumBySecondType(ccs.getId(),ishome,3);
	...
	public String listM1Json(){
		...
		int listCount = creativeService.creativeListControlGolfCount(sortId, secondSortId, title, userName, ishome);
		pageNo = (listCount+pageSize-1)/pageSize;
		creativeList = creativeService.creativeListControlGolf(sortId, secondSortId,title,userName,ishome, begin, pageSize);
						...
						if(c!=null){
							...
							if(tmpIsHome!=null){
								...
								if(ishome==5 && (tmpIsHome==2)){//在待审核状态 到已审核状态时
									//记录审核人
									NewsManageUser nmmu = this.getSessionGolfUser();
									 if(nmmu!=null){
										 c.setAuditUserId(nmmu.getId());
										 c.setStartTime(new Date());
									 }else{
