 ...
 * @version $id$
 */
@Results( { @Result(name = "upload_json", type = "json", params = { "root",
		"jsonResult", "contentType", "text/html" }) })
public class PublisherAction extends BaseAction {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1500225999969607339L;
	@Resource
	...
	public String updatePublisher() {
		try {
			...
			if(StringUtil.isBlank(publisher.getNormalContact())){
	...
	public String uploadLogoJson() {
		...
		try {
			...
			/*this.jsonResult = this.userService.uploadAvatar(publisher.getId(),
					logoFile, logoFileContentType, logoFileFileName);*/
			//暂用的userservice中的上传，名字要修改
